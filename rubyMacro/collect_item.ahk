...
runMacro(){
    ...
    try {
        ...
        response1 := MsgBox('Python not found`ndo you want to install python?', , 'YesNo')
        if (response1 = 'No'){
            MsgBox(e.Message)
            ExitApp()
        }
        MsgBox('You will be sent to a website`nplease download the latest version of python there')
